  ...
  return (
    <main className="pt-20 bg-[#fafaf9] min-h-screen">
          ...
          <div className="grid grid-cols-2 gap-0">
            ...
            <div className="pl-16 flex flex-col justify-between gap-7">
              ...
              <div className="flex gap-2.5">
                <a href="https://wa.me/919446516395" className="flex-1 inline-flex items-center justify-center gap-2 bg-[#111] text-white text-[10px] font-bold tracking-[0.2em] uppercase px-5 py-3.5 no-underline rounded-[3px] transition-colors hover:bg-[#C0001A]" style={{ fontFamily: "var(--font-inter, sans-serif)" }}>
      ...
      <section className="hidden md:block bg-[#fafaf9] border-t border-[#f0f0f0] py-20">
        <div className="max-container">
          <h2 className="text-[44px] font-normal leading-[1.1] text-[#111] mb-12" style={{ fontFamily: "var(--font-playfair, serif)" }}>Questions & <em className="text-[#C0001A]">Answers</em></h2>
          <div className="grid grid-cols-2 gap-x-16">
            {faqItems.map((item, i) => (
              <details key={item.q} className="group/faq border-b border-[#ebebeb] py-0">
                <summary className="py-6 text-[15px] font-medium text-[#111] cursor-pointer flex items-center justify-between list-none hover:text-[#C0001A] transition-colors" style={{ fontFamily: "var(--font-inter, sans-serif)" }}>
                   <span>{item.q}</span>
                   <ChevronDown size={16} />
                </summary>
                <div className="pb-6 text-sm text-[#666] leading-[1.8]" style={{ fontFamily: "var(--font-inter, sans-serif)" }}>{item.a}</div>
              </details>
            ))}
          </div>
        </div>
